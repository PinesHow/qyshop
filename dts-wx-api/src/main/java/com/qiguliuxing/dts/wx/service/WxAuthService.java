...
import com.qiguliuxing.dts.wx.util.PayUtil;
...
//            String shareAccessToken = "";//临时写法
            ...
            if (StringUtils.isEmpty(shareAccessToken)) {
                ...
                baseRedisService.setString("shareAccessToken",shareAccessToken,6660L);//暂时注释掉 部署的时候  打开
            }

            // redis 获取jsapiTicket  380*255=209425
...
//            String ticket = "";//临时写法
            ...
            if (StringUtils.isEmpty(ticket)) {
                ...
                baseRedisService.setString("jsapiTicket",ticket,6000L);//暂时注释掉 部署的时候  打开
            }
            ...
            e.printStackTrace();
            logger.error("获取签名异常,e:{}",e.getMessage());
    ...
    public String oauthToken(String code) throws Exception{
        return OauthTokenUtils.oauthToken(code,appId,appSecret);
    }
}
