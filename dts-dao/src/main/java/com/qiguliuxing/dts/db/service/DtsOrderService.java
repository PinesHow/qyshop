...
import com.qiguliuxing.dts.db.util.OrderTrackingUtils;
...
public class DtsOrderService {
	...
	public List<DtsOrder> querySelective(Integer userId, String orderSn, List<Short> orderStatusArray, Integer page,
		Integer size, String sort, String order, Integer shopId) {
		DtsOrderExample example = new DtsOrderExample();
	...
	public String orderTracking(DtsLogisticsTracking dtsLogisticsTracking) {
		try {
			String result = OrderTrackingUtils.getResult(dtsLogisticsTracking, "AKIDg6PhnL28jolskuSLVc627Z7xwGjjcxcUDwjE",
					"c85dh9yy8v7vnxor0a4c7ok6y81ser10h8l2gJdc", "market", "https://service-5ptj62bx-1257101137.ap-shanghai.apigateway.myqcloud.com/release/express/query");
			return result;
		}catch (Exception e){
			e.printStackTrace();
			return null;
	...
	public int createRefundTrace(DtsRefundTrace dtsRefundTrace) {
		dtsRefundTrace.setAddTime(LocalDateTime.now());
		dtsRefundTrace.setUpdateTime(LocalDateTime.now());
		dtsRefundTrace.setDeleted(false);
		return dtsOrderMapper.createRefundTrace(dtsRefundTrace);
	}

	public int updateRefundTraceByOrderId(DtsRefundTrace dtsRefundTrace) {
		dtsRefundTrace.setUpdateTime(LocalDateTime.now());
		dtsRefundTrace.setApproveTime(LocalDateTime.now());
		return dtsOrderMapper.updateRefundTraceByOrderId(dtsRefundTrace);
	}

	public DtsRefundTrace findRefundTraceByOrderId(Integer orderId) {
		return dtsOrderMapper.findRefundTraceByOrderId(orderId);
	}

	public DtsOrder findByPayId(String transactionId) {
		return dtsOrderMapper.findByPayId(transactionId);
	}

	public List<DtsOrder> selectOrderListByShopId(Integer shopId,Short orderStatus) {
		return dtsOrderMapper.selectOrderByShopId(shopId,orderStatus);
	}

	public DtsLogisticsResult selectLogisticsResultByOrderId(Integer orderId) {
		return dtsOrderMapper.selectLogisticsResultByOrderId(orderId);
	}

	public int addLogistics(DtsLogisticsResult dtsLogisticsResult) {
		return dtsOrderMapper.addLogistics(dtsLogisticsResult);
	}

	public int updateLogistics(DtsLogisticsResult dtsLogisticsResult) {
		return dtsOrderMapper.updateLogistics(dtsLogisticsResult);
	}
}
