...
public class DtsGoodsProductService {
	...
	public boolean queryDeletedFalseCountById(Integer productId) {
		DtsGoodsProductExample example = new DtsGoodsProductExample();
		example.or().andDeletedEqualTo(false).andIdEqualTo(productId);
		return ((int) dtsGoodsProductMapper.countByExample(example)) == 0;
	}
}