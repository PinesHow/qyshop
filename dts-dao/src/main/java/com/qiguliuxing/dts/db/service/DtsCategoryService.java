...
public class DtsCategoryService {
	...
	private DtsCategory.Column[] CHANNEL = { DtsCategory.Column.id, DtsCategory.Column.name,
			DtsCategory.Column.iconUrl,DtsCategory.Column.type };
	...
	public List<DtsCategory> queryChannel() {
		...
		List<DtsCategory> dtsCategoryList = categoryMapper.selectByExampleSelective(example, CHANNEL);
//		List<DtsCategory> dtsCategories = new ArrayList<>();
//		for(DtsCategory dtsCategory:dtsCategoryList){
//			Integer id = dtsCategory.getId();
//			Integer type = categoryMapper.selectTypeByCategoryId(id);
//			dtsCategory.setType(type);
//			dtsCategories.add(dtsCategory);
//		}
		return dtsCategoryList;
	}

	public List<DtsCategory> selectTree(Integer shopId) {
		return categoryMapper.selectTree(shopId);
	}

	public List<DtsCategory> queryPermissionByIds(List<Integer> idList,Integer shopType) {
		return categoryMapper.queryPermissionByIds(idList,shopType);
	}
}
