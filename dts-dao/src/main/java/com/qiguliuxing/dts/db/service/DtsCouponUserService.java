...
public class DtsCouponUserService {
	...
	public List<DtsCouponUser> queryExpiredTwo() {
		DtsCouponUserExample example = new DtsCouponUserExample();
		example.or().andStatusEqualTo(CouponUserConstant.STATUS_USABLE).andEndTimeEqualTo(LocalDate.now())
				.andDeletedEqualTo(false);
		return couponUserMapper.selectByExample(example);
	}

	public List<DtsCouponUser> queryShopCouponList(Integer userId, String shopId, Short status, Integer page, Integer size) {
		if (!StringUtils.isEmpty(page) && !StringUtils.isEmpty(size)) {
			PageHelper.startPage(page, size);
		}
		return couponUserMapper.selectShopCouponList(userId,shopId,status);
	}

	public List<DtsCouponUser> selectCouponUserByStatus(Integer userId, Integer shopId, Integer couponId, String orderSn,Short status) {
		DtsCouponUserExample example = new DtsCouponUserExample();
		DtsCouponUserExample.Criteria criteria = example.createCriteria();
		criteria.andStatusEqualTo(status).andUserIdEqualTo(userId).andShopIdEqualTo(shopId);
	...
	public int updateStatusAllByCouponIdList(Set<Integer> couponIdList, Integer userId,String orderSn) {
		return couponUserMapper.updateStatusAllByCouponIdList(userId,couponIdList,orderSn);
	}
}
