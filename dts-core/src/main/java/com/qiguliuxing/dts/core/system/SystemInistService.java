...
class SystemInistService {
	...
	private Map<String, String> getSystemInfo() {
		...
		infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 1, "后台服务相关");
...
//		infos.put("快递鸟ID", environment.getProperty("dts.express.appId"));
		infos.put("后台对象存储", environment.getProperty("dts.storage.active"));
		infos.put("后台本地对象存储路径", environment.getProperty("dts.storage.local.storagePath"));
		infos.put("后台本地对象访问地址", environment.getProperty("dts.storage.local.address"));
		infos.put("后台本地对象访问端口", environment.getProperty("dts.storage.local.port"));

		//微信相关信息
		infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 2, "微信相关");
		infos.put("微信本地对象访问地址", environment.getProperty("dts.storage.local.wxaddress"));
		infos.put("微信本地对象访问端口", environment.getProperty("dts.storage.local.wxport"));
