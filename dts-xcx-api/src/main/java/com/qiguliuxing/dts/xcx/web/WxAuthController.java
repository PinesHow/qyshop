...
import com.qiguliuxing.dts.xcx.dao.UserInfo;
import com.qiguliuxing.dts.xcx.dao.UserToken;
import com.qiguliuxing.dts.xcx.dao.WxLoginInfo;
import com.qiguliuxing.dts.xcx.service.CaptchaCodeManager;
import com.qiguliuxing.dts.xcx.service.UserTokenManager;
import com.qiguliuxing.dts.xcx.service.WxAuthService;
...
//	}
	...
	public Object login(@RequestBody WxLoginInfo wxLoginInfo, HttpServletRequest request) {
		logger.info("【请求开始】账户登录,请求参数,body:{}", wxLoginInfo);
		String openId = wxLoginInfo.getCode();
...
//			couponAssignService.assignForRegister(user.getId());
		...
		try {
			String registerDate = new SimpleDateFormat("yyyy-MM-dd")
					.format(user.getAddTime() != null ? Date.from( user.getAddTime().atZone( ZoneId.systemDefault()).toInstant()) : Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant()));
			userInfo.setRegisterDate(registerDate);
			userInfo.setStatus(user.getStatus());
			userInfo.setUserLevel(user.getUserLevel());// 用户层级
			userInfo.setUserLevelDesc(UserTypeEnum.getInstance(user.getUserLevel()).getDesc());// 用户层级描述
	...
	public Object loginByWeixin(@RequestBody WxLoginInfo wxLoginInfo, HttpServletRequest request) {
		...
		String code = wxLoginInfo.getCode();//微信小程序唯一标识
		UserInfo userInfo = wxLoginInfo.getUserInfo();//微信基本信息
...
//			couponAssignService.assignForRegister(user.getId());
	...
	/**
	 * 获取access_token(网页端) access_token安全级别高  不允许传递给前端
	 */
	@PostMapping("/oauthToken")
	public Object oauthToken(@LoginUser Integer userId, @RequestBody String body){
		logger.info("【请求开始】获取access_token(网页端),请求参数，body:{}", body);
		String code = JacksonUtil.parseString(body, "code");
		String result = "";
		try {
			/**
			 * TODO
			 * 如果access_token每次都是一样的  那么以下步骤缓存access_token和refesh_token 然后这步从redis获取access_token 如果access_token过期，
			 * 那么从缓存获取refesh_token调用刷新接口获取access_token,然后再缓存access_token
			 */
			result = OauthTokenUtils.oauthToken(code);
			String errcode = JacksonUtil.parseString(result,"errcode");
			if(!StringUtils.isEmpty(errcode)){//获取 access_token 失败
				return ResponseUtil.fail(500,"获取token失败:" + JacksonUtil.parseString(errcode,"errcode"));
			}
			String openId = JacksonUtil.parseString(result,"openid");
			String accessToken = JacksonUtil.parseString(result,"access_token");
			return getUserInfo(accessToken, openId);
		}catch (Exception e){
			logger.info("【请求异常】获取access_token(网页端),异常结果，e:{}",e.getMessage());
			return WxResponseUtil.fail(500,"获取token失败,请联系管理员"+e.getMessage());
		}
	}
...
//	@PostMapping("/getUserInfo")
	public Object getUserInfo(String accessToken,String openId){
		logger.info("请求开始】获取用户信息,请求参数，accessToken、openId:{}",accessToken,openId);
//		String accessToken = JacksonUtil.parseString(body,"accessToken");
//		String openId = JacksonUtil.parseString(body,"openId");
		System.out.println(accessToken);
		if(!StringUtils.isEmpty(accessToken) && !StringUtils.isEmpty(openId)){
			String result = "";
			try {
				result = OauthTokenUtils.getUserInfo(accessToken,openId);
			}catch (Exception e){
				logger.info("【请求异常】获取用户信息,异常结果，e:{}",e.getMessage());
				return WxResponseUtil.fail(500,"获取用户信息异常,请联系管理员"+e.getMessage());
			}
			return result;
		}
		return ResponseUtil.badArgument();
	...
	public Object getSignature(@Param("url") String url){
		if(StringUtils.isEmpty(url)){
			return ResponseUtil.badArgument();
		}
		return wxAuthService.getSignature(url);
	}
}
