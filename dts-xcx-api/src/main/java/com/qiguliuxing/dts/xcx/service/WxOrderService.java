...
import com.qiguliuxing.dts.xcx.dao.BrandCartGoods;
import com.qiguliuxing.dts.xcx.dao.BrandOrderGoods;
import com.qiguliuxing.dts.xcx.util.IpUtil;
import com.qiguliuxing.dts.xcx.util.WxResponseCode;
import com.qiguliuxing.dts.xcx.util.WxResponseUtil;
...
import static com.qiguliuxing.dts.xcx.util.WxResponseCode.*;
...
//		List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
		for (DtsOrder order : orderList) {
			...
			if(dtsOrderGoodsList != null){
				if(dtsOrderGoodsList.size() == 1){
					dtsOrderVo.setIsShopConsumption(true);
				}else{
					dtsOrderVo.setIsShopConsumption(false);
				}
			}else{
				return ResponseUtil.fail(500,"有异常订单，请联系管理员");
			}
			List<Integer> shopTypeList = orderGoodsService.selectShopTypeByOrderId(order.getId());
			if(StringUtils.isEmpty(shopTypeList)){
				return ResponseUtil.fail(500,"该订单号有异常，请联系管理员排查(可能原因，订单下的商品已不存在),订单号:"+order.getOrderSn());
			}
			if(shopTypeList.size() == 1 && shopTypeList.get(0).equals(3)){//单旅游产品
				dtsOrderVo.setType(1);
			}else if(shopTypeList.size() == 1 && shopTypeList.get(0).equals(2)){//单住宿产品
				dtsOrderVo.setType(1);
			}else{//混合产品 或 纯普通商品
...
//		}
		...
		if (SystemConfig.isMultiOrderModel()) {
			...
			if(brandCartgoodsList.size() == 1){//说明该购物车里的已选中商品属于一家店铺
				Integer shopId = brandCartgoodsList.get(0).getShopId();
...
//		order.setSettlementMoney(settlementMoney.setScale(2, BigDecimal.ROUND_DOWN));
		...
		// 删除购物车里面的商品信息
		if(cartId.equals(0)){//删除购物车里选中的商品
...
//					bandFreightPrice = SystemConfig.getFreight();
//				}
				childOrder.setGoodsPrice(bandGoodsTotalPrice);//设置商品总费用(同一家店铺)
				...
				childOrder.setOrderSn(order.getOrderSn() + "_" + num);
...
//			orderIds = orderIds + "," + order.getId().intValue();
		...
		for (DtsOrder dtsOrder : childOrderList) {
			...
			logger.info("商铺信息:"+orderShopInfo.toString());
...
//		try {
//			request.setTransactionId(order.getPayId());//微信订单号
//			request.setOutRefundNo(order.getOrderSn());//退款单号
//			BigDecimal orderPrice = order.getOrderPrice();//订单总金额(单位为元)
//			BigDecimal actualPrice = order.getActualPrice();//实付金额(单位为元)
//
//			String orderPriceString = orderPrice.multiply(new BigDecimal(100.00)).toString();
//			String actualPriceString = actualPrice.multiply(new BigDecimal(100.00)).toString();
//
//			request.setTotalFee(Integer.parseInt(orderPriceString.substring(0,orderPriceString.indexOf("."))));//订单总金额(单位为分)
//			request.setRefundFee(Integer.parseInt(actualPriceString.substring(0,actualPriceString.indexOf("."))));//退款金额(单位为分)
//			if(!StringUtils.isEmpty(refundReason)){//退款原因不为空
//				request.setRefundDesc(refundReason);
//			}
//			request.setNotifyUrl("");//回调函数地址
//			wxPayService.refund(request);
//		}catch (Exception e){
//			logger.error("订单申请退款失败",e.getMessage());
//			return WxResponseUtil.fail(ORDER_INVALID_OPERATION);
//		}
		// TODO 发送邮件和短信通知，这里采用异步发送
		// 有用户申请退款，邮件通知运营人员
...
//		}
		if(order.getOrderStatus().equals(OrderUtil.STATUS_SHIP)){//订单状态为发货状态
			...
			//先查询数据库持久化数据  如果没有  再通过接口查询
			DtsLogisticsResult dtsLogisticsResult = orderService.selectLogisticsResultByOrderId(orderId);
			if(StringUtils.isEmpty(dtsLogisticsResult)){//数据库未持久化该订单足迹
				request = orderService.orderTracking(dtsLogisticsTracking);
				//新增物流足迹
				DtsLogisticsResult dtsLogisticsResultTemp = new DtsLogisticsResult();
				dtsLogisticsResultTemp.setDeleted(false);
				dtsLogisticsResultTemp.setAddTime(nowTime);
				dtsLogisticsResultTemp.setUpdateTime(nowTime);
				dtsLogisticsResultTemp.setOrderId(orderId);
				dtsLogisticsResultTemp.setResult(request);
				orderService.addLogistics(dtsLogisticsResultTemp);
			}else{//数据库已持久化该订单足迹
				Duration duration = Duration.between(dtsLogisticsResult.getUpdateTime(),nowTime);
				long hours = duration.toHours();//相差的小时数
				if(hours > 5L){//上次查询 超过五小时
					request = orderService.orderTracking(dtsLogisticsTracking);
					dtsLogisticsResult.setResult(request);
					dtsLogisticsResult.setUpdateTime(nowTime);
					orderService.updateLogistics(dtsLogisticsResult);
				}else{//未超过五小时 则用数据库持久化的结果
					request = dtsLogisticsResult.getResult();
				}
			}
