...
import static com.qiguliuxing.dts.xcx.util.WxResponseCode.GOODS_NO_STOCK;
import static com.qiguliuxing.dts.xcx.util.WxResponseCode.GOODS_UNSHELVE;
...
//							return WxResponseUtil.fail(500,"该优惠券不适用所购商品");
						...
						if(type.equals(CouponUserConstant.STATUS_USABLE)){//通用券
							...
							if(flag == 1 || flag == 0){//满足满减
								...
								return WxResponseUtil.fail(500,"优惠券不满足满减");
							}
						...
						if(shopId.equals(shopIdTmp)){//优惠价格的商铺和当前店铺匹配
							bandGoodsTotalPriceTmp = bandGoodsTotalPriceTmp.subtract(disCount);
			...
			for (DtsCart cart : checkedGoodsList) {
				...
				if(!StringUtils.isEmpty(split) && split.length > 0 && Integer.parseInt(split[0]) != 0) {//使用优惠券
					...
					if(dtsCoupon == null){
						return WxResponseUtil.fail(500,"该优惠券不符合所购商品");
					}
					Short type = dtsCoupon.getType();
...
//		}
		...
		//统一保留两位小数,直接去掉两位小数以后的值
		grouponPrice = grouponPrice.setScale(2,BigDecimal.ROUND_UP);
		couponPrice = couponPrice.setScale(2,BigDecimal.ROUND_UP);
		orderTotalPrice = orderTotalPrice.setScale(2,BigDecimal.ROUND_UP);
		actualPrice = actualPrice.setScale(2,BigDecimal.ROUND_UP);
		// 返回界面的通用数据
